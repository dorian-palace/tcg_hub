...
<div class="min-h-screen bg-[rgb(17,24,39)] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('games.show', $game) }}" class="ml-1 text-sm font-medium text-gray-400 hover:text-white md:ml-2">{{ $game->name }}</a>
                        </div>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2">Modifier</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="bg-[rgb(31,41,55)] rounded-lg shadow-lg p-6">
            <h1 class="text-2xl font-bold text-white mb-6">Modifier le jeu</h1>
            ...
            <form method="POST" action="{{ route('games.update', $game) }}" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-700 rounded-md shadow-sm placeholder-gray-500 text-white bg-[rgb(31,41,55)] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm">{{ old('description', $game->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" id="website_url" name="website_url" value="{{ old('website_url', $game->website_url) }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-700 rounded-md shadow-sm placeholder-gray-500 text-white bg-[rgb(31,41,55)] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm">
                        @error('website_url')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
            ...
            <div class="mt-8 pt-8 border-t border-gray-700">
                <h2 class="text-xl font-bold text-white mb-4">Zone de danger</h2>
                <p class="text-sm text-gray-400 mb-4">Une fois supprimé, le jeu ne pourra pas être récupéré.</p>
                <form method="POST" action="{{ route('games.destroy', $game) }}" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce jeu ?')"
